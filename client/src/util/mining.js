...
export function parseMiningStats(log = '', algorithm = 'randomx') {
  ...
  if (algorithm === 'verushash') {
    ...
    const sv = /session:\s+([\d.]+)\s+VRSC/.exec(log)
    const su = /session:[^()]*\(\$([\d.]+)\)/.exec(log)
    const up = /uptime:\s+(\d+)s/.exec(log)
    ...
    return {
      ...
      sessionVrsc: sv ? Number(sv[1]) : undefined,
      sessionUsd: su ? Number(su[1]) : undefined,
      uptimeSec: up ? Number(up[1]) : undefined,
    }
  }
